"""
【案例】在 Milvus 向量库中做相似性检索（similarity_search_with_score）
...
- 运行前需确保 Milvus 中已有数据，例如先执行同目录下的 MilvusVectorStore.py；`collection_name`、连接配置也必须保持一致。
...
# 2. 连接已有集合（与 MilvusVectorStore.py 中 collection_name、连接配置一致）
